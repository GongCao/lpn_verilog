...
def evaluate(program_path):
    ...
    try:
        ...
        print("[DEBUG] Running 'make clean' before build...")
        rc_clean, out_clean = _run(["make", "clean"], cwd=SIM_DIR, timeout=MAKE_TIMEOUT_S)
        if rc_clean != 0:
            print("[DEBUG] 'make clean' failed (continuing anyway).")
            # Optional: uncomment if you want to see the log tail
            # print(out_clean[-1000:])
