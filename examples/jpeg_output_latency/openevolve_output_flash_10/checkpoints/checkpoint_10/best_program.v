// ============================================================================
// OpenEvolve seed for JPEG decoder latency optimization
// File name for evolution: initial_module.v
// IMPORTANT:
//   - Don't change the top module name and I/O
//   - Only modify code INSIDE the EVOLVE markers below, which is in module jpeg_output
//   - Do NOT add `#` delays or non-synthesizable constructs
// ============================================================================
...
// EVOLVE-BLOCK-START
// Determine Cb/Cr thresholds based on image mode
// For 4:2:0 mode, `level_o` in jpeg_output_cx_ram counts samples*4. Max 256 physical samples * 4 = 1024.
// To leave space for 64 physical samples (one block), which is 64*4 = 256 level units, threshold is 1024-256=768.
// For 4:4:4 mode, `level_o` counts actual samples. Max 256 physical samples.
// To leave space for 32 physical samples, threshold is 256-32=224.
wire [31:0] cb_cr_threshold_w = (img_mode_i == JPEG_YCBCR_420) ? 32'd768 : 32'd224;

// Y buffer threshold (always counts actual samples). Max 512 physical samples.
// To leave space for 64 physical samples (one block), threshold is 512-64=448.
localparam Y_THRESHOLD = 32'd448;

assign inport_accept_o   = (y_level_w <= Y_THRESHOLD &&
                          cb_level_w <= cb_cr_threshold_w &&
                          cr_level_w <= cb_cr_threshold_w) | idle_o;
...
begin
    ...
    else if (subsmpl_q != 2'b0) // 420: after the first block, assume data is available
        active_q <= 1'b1;
    else // 420: initial condition for first MCU (4 Y blocks, 1 Cb, 1 Cr)
...
// Wires to control individual FIFO pops, ensuring data validity
wire actual_y_pop_w;
wire actual_cb_pop_w;
wire actual_cr_pop_w;

always @ *
begin
    // `valid_r` indicates if the output stage is actively processing and has valid data to output.
    // This ensures `y_pop_w`, `cb_pop_w`, `cr_pop_w` are only asserted when data is actually present.
    valid_r  = active_q && y_valid_w && 
               (img_mode_i == JPEG_MONOCHROME ? 1'b1 : (cb_valid_w && cr_valid_w));
    r_conv_r = 32'b0;
    g_conv_r = 32'b0;
    b_conv_r = 32'b0;
    ...
    else // Covers YCbCr_444 and YCbCr_420 modes
    begin
...
assign actual_y_pop_w  = output_space_w && active_q && y_valid_w;
assign actual_cb_pop_w = output_space_w && active_q && cb_valid_w;
assign actual_cr_pop_w = output_space_w && active_q && cr_valid_w;

assign y_pop_w  = actual_y_pop_w;
assign cb_pop_w = actual_cb_pop_w;
assign cr_pop_w = actual_cr_pop_w;
