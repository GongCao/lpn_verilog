...
int main(int argc, char** argv) {
    ...
    size_t read = 0;
    size_t out_width, out_height, out_size=0, allocate_out_size = 0;
        ...
        if (!decoder->rst_i && !decoder->clk_i) {
            if (decoder->outport_valid_o) { // Caputure output data
                if (out_size == 0) {
                    ...
                    allocate_out_size = 64*ceil(out_width/8.0+1) * ceil(out_height/8.0+1);
                    out_r = new uint8_t[allocate_out_size];
                    out_g = new uint8_t[allocate_out_size];
                    out_b = new uint8_t[allocate_out_size];
                }
                ...
                if (pos == (out_size - 1)) {
                    VL_PRINTF("latency %" PRId64 " postion=%dX%d, exiting...\n",
