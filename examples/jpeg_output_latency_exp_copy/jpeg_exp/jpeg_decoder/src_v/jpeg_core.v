...
//-----------------------------------------------------------------
// LATENCY / SLEEP CONFIGURATION
//-----------------------------------------------------------------
// Change this value to increase/decrease latency.
// 0 = No extra latency
// 100 = Wait 100 clock cycles between valid pixels
// 1000000 = Wait ~0.01 seconds (at 100MHz)
localparam SLEEP_CYCLES = 32'd100; 

reg [31:0] sleep_counter_q;
wire       is_sleeping_w = (sleep_counter_q != 32'd0);

reg valid_r;
// Forward declaration of the internal valid signal
reg valid_q;

// MODIFIED: The pipeline can only move if we are NOT sleeping
// AND (we don't have valid data output pending OR the downstream accepted it)
wire output_space_w = (!valid_q || outport_accept_i) && !is_sleeping_w;
...
always @ (posedge clk_i)
if (rst_i)
    sleep_counter_q <= 32'd0;
else if (sleep_counter_q > 0)
    sleep_counter_q <= sleep_counter_q - 32'd1;
// If we have a valid pixel ready (valid_q), start the sleep timer
else if (valid_q && sleep_counter_q == 0)
    sleep_counter_q <= SLEEP_CYCLES;

//-----------------------------------------------------------------
// Outputs
//-----------------------------------------------------------------
// Note: valid_q declaration moved to top of file
...
// MODIFIED: Output valid is gated by sleep status
assign outport_valid_o   = valid_q && !is_sleeping_w;
